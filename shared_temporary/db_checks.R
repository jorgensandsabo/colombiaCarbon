  ...
  # Process model
    ...
    for(i in 1:nspecies){
      for(j in 1:nsites){
        ...
        # Observation process
        for(k in 1:ntrapdays){
          count[j,k,i] ~ dbin(d[i,hab[j]],abun[j,i])
        }
      }
    }
...
### Model checks
MCMCvis::MCMCplot()
MCMCvis::MCMCtrace(DBmod_OD,wd="C:\\Users\\Jorgen\\Desktop",filename="DBmod_Pois1.pdf")
...
DBmod_Pois <- rjags::jags.model(file = "JAGS\\Nmod_poisson.txt",
                              ...
                              inits =list(abun = apply(dbtab,c(1,3),sum)),
                              n.chains=3,n.adapt=1000)
update(object = DBmod_Pois, n.iter = 1000)
  ...
  # Process model
    ...
    for(i in 1:nspecies){
      for(j in 1:nsites){
        # Observation process
        for(k in 1:ntrapdays){
          abun[j,k,i] ~ dpois(lambda[i,hab[j]] * exp(ORLE[j,k,i]))
...
DBmod_OD <- rjags::jags.model(file = "JAGS\\Nmod_overdisp.txt",
                                ...
                                n.chains=3,n.adapt=1000)
update(object = DBmod_OD, n.iter = 1000)
