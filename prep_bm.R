#############################################
## DATA PREPARATION AND BIOMASS ESTIMATION ##
#############################################
# Code to prepare dataset and estimate biomass
...
if(jorgen.desktop){
  dir.path <- "C:\\Users\\Jorgen\\OneDrive - Norwegian University of Life Sciences\\PhD\\Data"
}

# Set working directory and load main files
setwd(dir.path)

library(dplyr)

Deaddata <- read.csv("raw\\Dead_clean.csv")
Grass <- read.csv("raw\\Grass_clean.csv")
Paramo <- read.csv("raw\\Paramo_clean.csv")
